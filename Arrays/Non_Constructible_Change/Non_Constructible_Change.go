...
This function works by first sorting the coins in ascending order. This is necessary to ensure that the function can efficiently find the minimum amount of change that you cannot create. The function then initializes the minimum amount of change that you cannot create.

The function then iterates over the coins. For each coin, the function checks if the minimum amount of change that you cannot create is less than the current coin. If so, then you can create all amounts of change up to and including the current coin. The function breaks from the loop.

Otherwise, you cannot create the minimum amount of change that you cannot create plus the current coin. The function adds the current coin to the minimum amount of change that you cannot create.

Once the function has finished iterating over the coins, it returns the minimum amount of change that you cannot create.
...
func minAmountOfChange(coins []int) int {
	// Sort the coins in ascending order.
	sort.Ints(coins)

	// Initialize the minimum amount of change that you cannot create.
	minAmountOfChange := 1

	// Iterate over the coins.
	for _, coin := range coins {
		// If the minimum amount of change that you cannot create is less than the current coin,
		// then you can create all amounts of change up to and including the current coin.
		if minAmountOfChange < coin {
			break
		}

		// Otherwise, you cannot create the minimum amount of change that you cannot create
		// plus the current coin.
		minAmountOfChange += coin
	}

	// Return the minimum amount of change that you cannot create.
	return minAmountOfChange
}

func main() {
	coins := []int{5, 7, 1, 1, 2, 3, 22}

	minAmountOfChange := minAmountOfChange(coins)

	fmt.Println(minAmountOfChange)
}
