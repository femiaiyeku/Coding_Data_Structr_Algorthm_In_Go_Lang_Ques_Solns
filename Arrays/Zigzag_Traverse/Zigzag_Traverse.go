...
func zigzagOrder(array [][]int) []int {
	// Create an empty one-dimensional array to store the results.
	result := []int{}
	...
	// Iterate over the two-dimensional array in a zigzag pattern.
	direction := "down"
	for row < len(array) && column < len(array[0]) {
		// Add the current element to the results.
		result = append(result, array[row][column])

		// Move to the next element in the zigzag pattern.
		if direction == "down" {
			if row+1 < len(array) {
				row++
			} else {
