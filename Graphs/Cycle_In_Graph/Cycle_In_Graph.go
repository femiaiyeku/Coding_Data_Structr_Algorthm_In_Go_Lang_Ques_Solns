...
func detectCycle(edges [][]int) bool {
    ...
    for i := range edges {
        if !visited[i] {
            if dfs(edges, i, visited, recursionStack) {
                return true
            }
        }
    }
...
func dfs(edges [][]int, currentNode int, visited map[int]bool, recursionStack map[int]bool) bool {
    ...
    for _, neighbor := range edges[currentNode] {
        if dfs(edges, neighbor, visited, recursionStack) {
            return true
        }
    }
