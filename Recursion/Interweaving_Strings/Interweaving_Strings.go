...
func canInterweave(one, two, three string) bool {
	...
	for i := 0; i < len(three); i++ {
		if oneIndex < len(one) && three[i] == one[oneIndex] {
			oneIndex++
		} else if twoIndex < len(two) && three[i] == two[twoIndex] {
			twoIndex++
		} else {
			return false
		}
...
func main() {
	...
	canInterweave := canInterweave(one, two, three)
	fmt.Println(canInterweave)
}
